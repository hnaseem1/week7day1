...
# call[:response][:results][0][:views] = 0
# p call[:response][:results][0].keys

@articles = @call[:response][:results] # is an array of hashes

@articles.each do |article_hash| # method that loops through each hash and adds a key and a value

  article_hash[:views] = 0

end

def read_article # mathod that randomly selects an article and increment the :views key by 1 each time the article is selected
...
def display_views # method displays the article names and views

  @articles.each do |article_hash|
    p "Article: #{article_hash[:webTitle]} -- views: #{article_hash[:views]}"
  end

end


# called several times to check if the methods above work
